...
#require('visNetwork')
#
nodes <- read.csv("NODES_McKQuarterly.csv", header=T, as.is=T)
links <- read.csv("EDGES_McKQuarterly.csv", header=T, as.is=T)
nodes_1 = as.data.frame(nodes$id)
nodesU=unique(nodes_1)

head(nodesU)
head(links)
#nrow(nodesU); length(unique(nodesU$id))  Not Reqd Now We Go Unique of First Column 
#
# Gives Total number of ROWS in CSV 
# Also gives Unique Number of NODES in CSV 
...
#
net <- graph_from_data_frame(d=links, vertices=nodesU, directed=T) 
net
#
...
write.csv(IMXX, file = "IM_McKQuarterly.csv", row.names = TRUE)
#
...
write.csv(AMXX, file = "AM_McKQuarterly.csv", row.names = TRUE)
#
#
#
# NOT Being Done Below this .....
#
#
